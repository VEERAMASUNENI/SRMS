...
// ── Tokenize ───────────────────────────────────────
function tokenize(text, wordMode) {
  let tokens;
  if (wordMode) {
...
function normalizeTokens(tokens, caseSensitive) {
  return caseSensitive ? tokens : tokens.map(t => t.toLowerCase());
}
...
function analyze() {
  ...
  const tokensA = tokenize(rawA, wordMode);
  const tokensB = tokenize(rawB, wordMode);
  const compareA = normalizeTokens(tokensA, caseSensitive);
  const compareB = normalizeTokens(tokensB, caseSensitive);

  const dpCells = tokensA.length * tokensB.length;
  if (dpCells > MAX_DP_CELLS) {
    flash(`Input is too large for the full DP matrix (${dpCells.toLocaleString()} cells). Try shorter text or switch modes.`, 'warning');
    return;
  }

  // Guard against long inputs in char mode
  ...
  const { dp, lcs, pathSet } = computeLCS(compareA, compareB);
  const elapsed = performance.now() - t0;

  // Store state
  state = { tokensA, tokensB, compareA, compareB, dp, lcs, pathSet, computed: true, animating: false, animRevealCount: 0 };
  ...
  if (lcs.length === 0) {
    ...
    lcs.forEach((item, idx) => {
      ...
      span.textContent = tokensA[item.idxA];
...
function renderMatrix() {
  ...
  const { tokensA, tokensB, compareA, compareB, dp, pathSet } = state;
  ...
  const cmpA = compareA.slice(0, limit);
  const cmpB = compareB.slice(0, limit);
  ...
  for (let i = 1; i <= rows; i++) {
    ...
    for (let j = 0; j <= cols; j++) {
      ...
      const isMatch = j > 0 && cmpA[i - 1] === cmpB[j - 1];
